...
class NovelSettingsActivity : AppCompatActivity() {
    ...
    private val autoImageHelp =
        "When this is on, the app makes pictures for some chapters of this novel. " +
            "It keeps going while you are in the app or while it is reading aloud. " +
            "That includes a novel you marked as read or one " +
            ...
            "It waits between pictures as set in Settings, and skips a chapter that already " +
    ...
    private val redownloadHelp =
        "Replaces this novel on your phone with a fresh download from the website. Any translations are lost, and would cost money to make again."
    ...
    private fun bindAutoDownload(folder: String, state: State) {
        ...
        autoDownloadHelp = if (finished) {
            "This novel is finished, so there are no new chapters to download."
        } else {
            "Automatically download new chapters of this novel when they come out."
        }
    ...
    private fun bindTranslate(folder: String, state: State) {
        ...
        translateHelp = when {
            english -> "This novel is already in English, so there is nothing to translate."
            allTranslated ->
                "Every chapter is already translated. New chapters will be translated " +
                    "too while this is on. This costs money, charged to your Anthropic account."
            else ->
                "Translate this novel into English. This costs money, charged to your Anthropic account."
        }
        box.setOnCheckedChangeListener { _, checked ->
    ...
    private fun deleteChapters(folder: String, dirNow: String): Deleted {
        ...
        for (kid in kids) {
            ...
            if (!isChapter && !(kid.isDir && (kid.name == "translated" || kid.name == Scenes.DIR))) continue
            val ok = try {
