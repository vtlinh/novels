-- schema of version 27, as onCreate builds it
-- extracted to match Schema.create; do not hand-edit
CREATE TABLE chapters (folder TEXT, slug TEXT, filename TEXT, uri TEXT, url TEXT DEFAULT '', size INTEGER DEFAULT 0, hash TEXT DEFAULT '', image TEXT DEFAULT '', image_alt TEXT DEFAULT '', slack_thread TEXT DEFAULT '', PRIMARY KEY(folder, slug, filename));
...
CREATE TABLE IF NOT EXISTS chapter_image_req (folder TEXT, slug TEXT, chapter TEXT, hash TEXT DEFAULT '', thread_ts TEXT DEFAULT '', started_at INTEGER DEFAULT 0, looked INTEGER DEFAULT 0, PRIMARY KEY(folder, slug, chapter, thread_ts));
CREATE TABLE IF NOT EXISTS chapter_image (folder TEXT, slug TEXT, chapter TEXT, image TEXT DEFAULT '', alt TEXT DEFAULT '', PRIMARY KEY(folder, slug, chapter));
